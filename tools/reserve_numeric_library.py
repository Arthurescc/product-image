#!/usr/bin/env python3
"""Pre-register stable URLs for a numeric photo range."""

from __future__ import annotations

import argparse
import json
from datetime import datetime, timezone
from pathlib import Path
from urllib.parse import quote


DEFAULT_BATCH_ID = "photos-1-15000"
DEFAULT_BATCH_NAME = "Product Image Collection"
DEFAULT_START = 1
DEFAULT_END = 15_000


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--repo", default="Arthurescc/product-image")
    parser.add_argument("--tag", default=DEFAULT_BATCH_ID)
    parser.add_argument("--batch-id", default=DEFAULT_BATCH_ID)
    parser.add_argument("--batch-name", default=DEFAULT_BATCH_NAME)
    parser.add_argument("--start", type=int, default=DEFAULT_START)
    parser.add_argument("--end", type=int, default=DEFAULT_END)
    parser.add_argument("--catalog", type=Path, default=Path("docs/data/catalog.json"))
    args = parser.parse_args()

    if args.start < 1 or args.end < args.start:
        raise SystemExit("start and end must define a positive numeric range")

    catalog = {
        "repositoryUrl": f"https://github.com/{args.repo}",
        "generatedAt": None,
        "batches": [],
        "assets": [],
    }
    if args.catalog.exists():
        try:
            catalog.update(json.loads(args.catalog.read_text(encoding="utf-8")))
        except (json.JSONDecodeError, OSError):
            pass

    existing = {
        item.get("id"): item
        for item in catalog.get("assets", [])
        if item.get("batchId") == args.batch_id
    }
    release_url = f"https://github.com/{args.repo}/releases/tag/{quote(args.tag, safe='')}"
    owner, repository = args.repo.split("/", 1)
    pages_base = f"https://{owner.lower()}.github.io/{repository}/thumbnails/{args.batch_id}"
    assets = []
    for number in range(args.start, args.end + 1):
        source_name = f"{number}.jpg"
        original_name = source_name
        asset_id = f"{args.batch_id}-{number:05d}"
        prior = existing.get(asset_id, {})
        status = "ready" if prior.get("status") == "ready" else "reserved"
        assets.append(
            {
                "id": asset_id,
                "title": source_name,
                "batchId": args.batch_id,
                "batchName": args.batch_name,
                "sourcePath": source_name,
                "width": int(prior.get("width") or 0),
                "height": int(prior.get("height") or 0),
                "thumbnail": prior.get("thumbnail") or "",
                "originalName": original_name,
                "originalUrl": f"{pages_base}/{number:05d}.webp",
                "archiveName": prior.get("archiveName") or "待上传 ZIP",
                "archiveUrl": prior.get("archiveUrl") or release_url,
                "releaseTag": args.tag,
                "status": status,
            }
        )

    ready_count = sum(item["status"] == "ready" for item in assets)
    batch = {
        "id": args.batch_id,
        "name": args.batch_name,
        "count": len(assets),
        "readyCount": ready_count,
        "archiveName": "一个或多个 ZIP（后续上传）",
        "archiveUrl": release_url,
        "releaseTag": args.tag,
        "status": "ready" if ready_count == len(assets) else "indexing",
    }

    catalog["repositoryUrl"] = f"https://github.com/{args.repo}"
    catalog["batches"] = [item for item in catalog.get("batches", []) if item.get("id") != args.batch_id] + [batch]
    catalog["assets"] = [item for item in catalog.get("assets", []) if item.get("batchId") != args.batch_id] + assets
    catalog["generatedAt"] = datetime.now(timezone.utc).isoformat()
    args.catalog.parent.mkdir(parents=True, exist_ok=True)
    args.catalog.write_text(json.dumps(catalog, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(f"Pre-registered {len(assets)} fixed photo URLs")
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
